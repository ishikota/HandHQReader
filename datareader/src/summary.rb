class Summary
  ...
  def read_pot(line)
    pot = line.match(/Total Pot\(\$([\d\.]+)\)/)[1].to_f
    rake = line.match(/Rake \(\$([\d\.]+)\)/)
    jackpot_rake = line.match(/Jackpot Rake \(\$([\d\.]+)\)/)
    rake = rake.nil? ? 0 : rake[1].to_f
    jackpot_rake = jackpot_rake.nil? ? 0 : jackpot_rake[1].to_f
  ...
  class Result
    ...
    def read_detail(src)
      if md = src.match(/ collected Total \(\$[\d\.]+\)/)
        [ COLLECT, { "amount" => md[1].to_f } ]
      elsif md = src.match(/Folded on the (.*)/)
        [ FOLD, { "street" => md[1] } ]
      elsif md = src.match(/ won Total \(\$([\d\.]+)\) .* with (.*?),(.*)/)
        [ WON, { "amount" => md[1].to_f, "hand" => md[2], "detail" => md[3] } ]
      elsif md = src.match(/\[Mucked\] \[(.*)\]/)
        [ MUCK, { "hole" => md[1].split } ]
      elsif md = src.match(/lost with (.*?), (.*)/)
        [ LOST, { "hand" => md[1], "detail" => md[2] } ]
      else
